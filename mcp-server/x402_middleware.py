...
class X402PaymentMiddleware:
    ...
    async def _send_402(self, send):
        ...
        try:
            ...
            # Tempo testnet PathUSD for now (mainnet USDC requires token init)
            # TODO: Switch to mainnet USDC once Tempo mainnet token init is resolved
            TEMPO_PATH_USD = "0x20c0000000000000000000000000000000000000"
            TEMPO_CHAIN_ID = 42431  # Testnet (Moderato)
            tempo_recipient = os.environ.get("TEMPO_PAY_TO_ADDRESS", single_req["payTo"])

            mpp_request = {
                "amount": "10000",  # 0.01 PathUSD in base units (6 decimals)
